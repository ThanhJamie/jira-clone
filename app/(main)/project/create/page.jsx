...
export default function CreateProjectPage() {
  ...
  useEffect(() => {
    if (isOrgLoaded && isUserLoaded) {
      setIsAdmin(membership?.role === "org:admin" || false);
    }
  ...
  // Skip organization check on client side, let server handle it
  if (!isAdmin && isOrgLoaded && isUserLoaded) {
    return (
      <div className="flex flex-col gap-4 items-center justify-center min-h-screen">
        <span className="text-3xl gradient-title text-center">
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Project key must be unique within your organization
          </p>
        </div>
        <div>
          <Textarea
